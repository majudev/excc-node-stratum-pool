...
var pool = module.exports = function pool(options, authorizeFn){
    ...
    function DetectCoinData(finishedCallback){
        ...
        //stage 1 - getdifficulty
        _this.daemon.cmd('getdifficulty', [], function(getdifficulty_results){
            if(getdifficulty_results[0] == 'undefined' || getdifficulty_results[0].error){
                emitErrorLog('Could not start pool, error with init RPC getdifficulty - ' + JSON.stringify(getdifficulty_results[0].error));
                return;
            }

            if (!options.coin.reward) {
                if (isNaN(getdifficulty_results[0].response) && 'proof-of-stake' in getdifficulty_results[0].response)
                    options.coin.reward = 'POS';
                else
                    options.coin.reward = 'POW';
            }

            //stage 2 - validateaddress
            _this.daemon.cmd('validateaddress', [options.address], function(validateaddress_results){
                if(validateaddress_results[0] == 'undefined' || validateaddress_results[0].error){
                    emitErrorLog('Could not start pool, error with init RPC validateaddress - ' + JSON.stringify(validateaddress_results[0].error));
                    return;
                }

                if (!validateaddress_results[0].response.isvalid){
                ...
                if (options.coin.reward === 'POS' && typeof(validateaddress_results[0].response.pubkey) == 'undefined') {
                ...
                options.poolAddressScript = (function(){
                    switch(options.coin.reward){
                        case 'POS':
                            return util.pubkeyToScript(validateaddress_results[0].response.pubkey);
                        case 'POW':
                            return util.addressToScript(validateaddress_results[0].response.address);
                    }
                })();

                //stage 3 - getinfo
                if (options.coin.hasGetInfo) {
                    _this.daemon.cmd('getinfo', [], function(getinfo_results){
                        if(getinfo_results[0] == 'undefined' || getinfo_results[0].error){
                            emitErrorLog('Could not start pool, error with init RPC getinfo - ' + JSON.stringify(getinfo_results[0].error));
                            return;
                        }
                        options.testnet = getinfo_results[0].response.testnet;
                        options.protocolVersion = getinfo_results[0].response.protocolversion;
                        var difficulty = getinfo_results[0].response.difficulty;
                        if (typeof(difficulty) == 'object') {
                            difficulty = difficulty['proof-of-work'];
                        }

                        //stage 4 - getmininginfo
                        _this.daemon.cmd('getmininginfo', [], function(getmininginfo_results){
                            if(getmininginfo_results[0] == 'undefined' || getmininginfo_results[0].error){
                                emitErrorLog('Could not start pool, error with init RPC getmininginfo - ' + JSON.stringify(getmininginfo_results[0].error));
                                return;
                            }

                            options.initStats = {
                                connections: getinfo_results[0].response.connections,
                                difficulty: difficulty * algos[options.coin.algorithm].multiplier,
                                networkHashRate: getmininginfo_results[0].response.networkhashps
                            };

                            //stage 5 - submitblock
                            _this.daemon.cmd('submitblock', [], function(submitblock_results){
                                if (submitblock_results[0] != 'undefined' && submitblock_results[0].error != null && submitblock_results[0].error.message === 'Method not found'){
                                    options.hasSubmitMethod = false;
                                }
                                else if (submitblock_results[0] != 'undefined' && submitblock_results[0].error != null && (submitblock_results[0].error.code === -1 || submitblock_results[0].error.code === -8)){
                                    options.hasSubmitMethod = true;
                                }
                                else {
                                    emitErrorLog('Could not detect block submission RPC method, ' + JSON.stringify(submitblock_results[0]));
                                    return;
                                }
                    ...
                    _this.daemon.cmd('getblockchaininfo', [], function(getblockchaininfo_results){
                        if(getblockchaininfo_results[0] == 'undefined' || getblockchaininfo_results[0].error){
                            emitErrorLog('Could not start pool, error with init RPC getblockchaininfo - ' + JSON.stringify(getblockchaininfo_results[0].error));
                            return;
                        }

                        _this.daemon.cmd('getnetworkinfo', [], function(getnetworkinfo_results){
                            if(getnetworkinfo_results[0] == 'undefined' || getnetworkinfo_results[0].error){
                                emitErrorLog('Could not start pool, error with init RPC getnetworkinfo - ' + JSON.stringify(getnetworkinfo_results[0].error));
                                return;
                            }
                            
                            options.testnet = (getblockchaininfo_results[0].response.chain === 'test') ? true : false;
                            options.protocolVersion = getnetworkinfo_results[0].response.protocolversion;
                            var difficulty = getblockchaininfo_results[0].response.difficulty;
                            if (typeof(difficulty) == 'object') {
                                difficulty = difficulty['proof-of-work'];
                            }
                            ...
                            _this.daemon.cmd('getmininginfo', [], function(getmininginfo_results){
                                ...
                                options.initStats = {
                                    connections: getnetworkinfo_results[0].response.connections,
                                    difficulty: difficulty * algos[options.coin.algorithm].multiplier,
                                    networkHashRate: getmininginfo_results[0].response.networkhashps
                                }
