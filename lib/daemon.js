 ...
 * - 'https'   : true/false
 * - 'ca'      : path to the server's CA file
 * - 'noverify': do not verify https against CA cerificate
**/
...
function DaemonInterface(daemons, logger){
    ...
    function performHttpRequest(instance, jsonData, callback){
        var options = {
            ...
            ca      : [ fs.readFileSync(instance.ca) ],
            rejectUnauthorized: instance.noverify
        };
	...
	if (instance.https)
		var http_engine = https;
	else
		var http_engine = http;

        var req = http_engine.request(options, function(res) {
            var data = '';
